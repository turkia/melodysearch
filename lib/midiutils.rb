...
class MidiUtils
	...
	def MidiUtils.notename_to_midival(notename, lang)
		return nil if not notename =~ /^([cCdDeEfFgGaAbBhH])(is|es|#){0,1}([0-9])$/ 
	...
	def MidiUtils.midival_to_notename(midival, lang)
		return "#{@@names_en[midival % 12]}#{midival / 12}"
	end
	...
	def MidiUtils.merge(ifile_dir, ofile, ofiledivision)
		...
		if ifile_dir then ifile_dir = ifile_dir.dup.gsub(/\/\//, '/') end

		if ifile_dir and File.directory?(ifile_dir) and ifile_dir !~ /\.$/ then
			Dir.foreach(ifile_dir) do |file|
				if file =~ /\.mid$/ then
					maxoffset = 0

					begin
					sq = Sequence.decodefile(ifile_dir + File::SEPARATOR + file)
